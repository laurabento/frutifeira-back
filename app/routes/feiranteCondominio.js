  ...
  .delete(authorize(), async (req, res) => {
    ...
    if (id.match(/^[0-9a-fA-F]{24}$/)) {
      ...
      try {
        ...
        res
          .status(200)
          .json({ message: "O condomínio foi deletado com sucesso!" });
      } catch (error) {
        console.log(error);
      }
    } else {
...
router
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      res
        .status(201)
        .json({ message: "Solicitação enviada com sucesso!", condo: newCondo });
    } catch (error) {
      console.log(error);
    }
  });
...
router.route("/nome/:search").get(authorize(), async (req, res) => {
  ...
  try {
    ...
    if (!condos || condos.length === 0) {
      res
        .status(404)
        .json({ error: "Nenhum condomínio encontrado com esse nome!" });
      return;
    } else res.status(200).json(condos);
  } catch (error) {
    console.log(error);
  }
});

router.route("/condominio/:id").get(async (req, res) => {
  // #swagger.tags = ['FeiraCondominio']
  const id = req.params.id;
  try {
    console.log(id);
    ...
    if (!marketCondos || marketCondos.length === 0) {
      res
        .status(404)
        .json({ error: "Nenhum condomínio encontrado com esse Id!" });
      return;
    } else {
      var marketVendorsIds = [];
...
router.route("/condominio/:id/aprovado").get(async (req, res) => {
  // #swagger.tags = ['FeiraCondominio']
  const id = req.params.id;
  try {
    console.log(id);
...
router.route("/condominio/:id/pendente").get(async (req, res) => {
  // #swagger.tags = ['FeiraCondominio']
  const id = req.params.id;
  try {
    console.log(id);
    var validStatus = [];
    validStatus.push("Aprovado");
    validStatus.push("Reprovado");
    validStatus.push("Pendente");
    var marketCondos = await MarketCondominium.find({ condominiumId: id, status: { $in: validStatus } });
    console.log(marketCondos);
    if (!marketCondos || marketCondos.length === 0) {
      res
        .status(404)
...
router.route("/condominio/:id/produtos").get(async (req, res) => {
  // #swagger.tags = ['FeiraCondominio']
  const id = req.params.id;
  try {
    console.log(id);
...
router.route("/feirante/:id").get(authorize(), async (req, res) => {
  // #swagger.tags = ['FeiraCondominio']
  const id = req.params.id;
  try {
...
router.route("/feirante/:id/aprovado").get(authorize(), async (req, res) => {
  // #swagger.tags = ['FeiraCondominio']
  console.log("aprovados");
  const id = req.params.id;
  try {
    const marketCondos = await MarketCondominium.find({
      marketVendorId: id,
      status: "Aprovado",
    });
