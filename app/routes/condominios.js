...
router
  ...
  .post(async (req, res) => {
    ...
    try {
      ...
      res.status(201).json({
        message: "O condomínio foi inserido com sucesso!",
        condo: newCondo,
      });
    } catch (error) {
      console.log(error);
...
router.route("/login").post(async (req, res) => {
  ...
  try {
    ...
    if (
      existingCondo &&
      (await bcrypt.compare(password, existingCondo.password))
    ) {
...
router.route("/endereco/:search").get(authorize(), async (req, res) => {
  ...
  try {
    ...
    if (!condos || condos.length === 0) {
      res
        .status(404)
        .json({ error: "Nenhum condomínio encontrado com esse endereço!" });
      return;
    } else res.status(200).json(condos);
  } catch (error) {
    console.log(error);
